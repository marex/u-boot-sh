...
static void rpc_spi_invalidate_cache(struct udevice *dev, u32 offset, u32 len)
{
	struct udevice *bus = dev->parent;
	struct rpc_spi_priv *priv = dev_get_priv(bus);
	fdt_addr_t start = round_down(priv->extr + offset, ARCH_DMA_MINALIGN);
	fdt_addr_t end = round_up(priv->extr + offset + len, ARCH_DMA_MINALIGN);
			...
			const void *dout, void *din, unsigned long flags)
{
	...
	if (wlen || (!rlen && !wlen) || flags == SPI_XFER_ONCE) {
		...
		rpc_spi_invalidate_cache(dev, offset, rlen);
